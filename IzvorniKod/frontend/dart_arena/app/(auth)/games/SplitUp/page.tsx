...
export default function SplitUp() {
  ...
  const [clicked, setClicked] = useState<number[]>([0, 1, 2]);
  const targets = [
    "15s",
    "16s",
    "DOUBLES",
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (localStorage.length < 3) {
        router.push("/games/startSplitUp");
      }
      ...
      if (settings && userQuery.isFetched) {
        ...
        newPlayer1 = {
          ...
          ["score"]: historyLast?.playerFirst.score || 40,
          ["threeDartAverage"]:
        ...
        if (
          ...
          if (data !== "error") {
            newPlayer2 = {
              ...
              ["score"]: historyLast?.playerSecond.score || 40,
              ["threeDartAverage"]:
  ...
  const handleScore = () => {
    ...
    const legCount =
      history.filter(
        (h) => h.playerFirst.score === 40 && h.playerSecond.score === 40
      ).length - 1;
    ...
    if (player1.onTurn && player2.name !== "") {
      let newValue;
      if (userScore === 0) newValue = player1.score / 2;
      else newValue = player1.score + userScore;
      ...
      newHistory = [
        ...history,
        {
          playerFirst: {
            ...player1,
            score:
              r === 9 && player1.dartsUsed + 3 === player2.dartsUsed
                ? 40
                : newValue,
            onTurn:
              r === 9 && player1.dartsUsed + 3 === player2.dartsUsed
      ...
      checkEnd(rd, newHistory);
    } else if (player2.onTurn && player2.name !== "") {
      let newValue;
      if (userScore === 0) newValue = player2.score / 2;
      else newValue = player2.score + userScore;
      ...
      newHistory = [
        ...history,
        {
          playerFirst: {
            ...player1,
            onTurn:
              r === 9 && player2.dartsUsed + 3 === player1.dartsUsed
                ? ((legCount + 1) % 2 === 1 &&
                    gameSettings.startPlayerNum == 2) ||
                  ((legCount + 1) % 2 === 0 && gameSettings.startPlayerNum == 1)
                  ? true
      ...
      newHistory = [
        ...history,
        {
          playerFirst: {
            ...
            dartsUsed: r === 9 ? 0 : player1.dartsUsed + 3,
            legsWon:
              r === 9
                ? player1.legsWon + 1 == gameSettings.legs
                  ? 0
                ...
                : player1.legsWon,
            setsWon:
              r === 9 && player1.legsWon + 1 == gameSettings.legs
                ? player1.setsWon + 1
                : player1.setsWon,
            dartsUsedPerLeg:
              r === 9
  ...
  const checkEnd = (r: number, newHistory: any) => {
    ...
    } else if (r < 10 && (p1 === p2 || player2.name === "")) {
      setRound([...round, r]);
  ...
  const handleMatchData = (latest: any) => {
    ...
    const data = {
      ...
      checkoutPercentage:
        player2.name === ""
          ? [100, 0]
          : calculateCheckoutPercentage(history, gameSettings, latest, 40),
      score60Plus: [0, 0],
      score100Plus: [0, 0],
      score140Plus: [0, 0],
      score170: [0, 0],
    ...
    apiCall(`/user/validate`, {
      ...
      .then(([data, status]) => {
        if (status === 200) {
          ...
          setPasswdErr("");
          if (formData) submitMatchData(formData, "startSplitUp", router);
          ...
          <div className="w-full flex gap-2 pl-2 pr-2">
            ...
            <div className="w-full h-13 bg-background/50 flex justify-center items-center gap-3 rounded-lg p-2">
              {userInput.map((s: string, i: number) => (
                <div key={clicked[i]}>{s}</div>
              ))}
              {userInput.length === 0 && "-"}
            </div>
          </div>
          <div className="flex flex-wrap justify-center items-center gap-2 w-full">
            {splitInput[round[round.length - 1] - 1].map((s: string) => (
              <div
                key={s}
                onClick={
              ...
              >
                <p className={s === "fake" ? "opacity-0" : ""}>{s}</p>
